...
app._initializeHTTP = function() {
  ...
  this.extractClients = function() {
    console.log(this._clients);
    if (this._clients.peerjs instanceof Object && this._clients.peerjs) {
      return Object.keys(this._clients.peerjs).slice(0, 20);
    } else {
      return [];
    }
  }

  this.get('/ids', function (req, res, next) {
    res.send(self.extractClients());
  });

  // Retrieve guaranteed random ID.
  this.get('/:key/id', function(req, res, next) {
    res.contentType = 'text/html';
    var randomIdd = self._generateClientId(req.params.key);
    console.log('id generated', randomIdd);
    ids.push(randomIdd);
    res.send(randomIdd);
  });
